...
const Pages = () => {
  ...
  return (
    <DefaultLayout>
      ...
      <Marking mode={learnMode}>平均情報量(エントロピー)</Marking>です。
      ...
      <Box>
        この平均情報量を利用して、特徴量の分類によってどれだけ情報を獲得できたのかを評価するのです。
        このとき、獲得できた情報量を
        <Marking mode={learnMode}>情報ゲイン</Marking>
        といいます。 情報ゲインを言葉を用いた式で表すと
        <Display>
          {"\\(情報ゲイン=分類前の平均情報量-分類後の平均情報量の期待値\\)"}
        </Display>
        ...
        実際に各特徴量による分類にでどれだけ情報量を獲得できたのか計算してみましょう。
      </Box>
      ...
      次に、性別によってどれだけの情報量を獲得できたかを計算しましょう。平均情報量の
      計算方法はルートノードのときと同じです。
      <Display>
        {`\\( \\begin{align}
      ...
      また終了条件は分類後のノードの教師信号がすべて同じクラスに属するとき、もしくは、分類に使う
      特徴量がなくなったときです。特徴量がなくなった場合は、分類しきれていない場合があります。そのときは
      そのノードの教師信号で多数決をとります。上で示した、決定木も多数決で決めていることがわかります。
